...
\begin{figure}[H]
    \centering
    \subfloat[]{\includegraphics[width=.5\linewidth]{Imgs/Sparse Linear Regression/11_population_error_log_all.png}\label{fig:l1poperrorlog}}\hfill
    \subfloat[]{\includegraphics[width=.5\linewidth]{Imgs/Sparse Linear Regression/11_population_error_all.png}\label{fig:l1poperror}}
    \caption{The excess $\ell^1$ population error of the gradient descent solution, $\boldsymbol{\beta}_{\boldsymbol{w}_{\alpha}(t_{\text{end}})}$, as a function of the number of training samples $n$ and the initialization scale $\alpha$. A dashed line indicates that the gradient descent path with initialization $\boldsymbol{w}_{\alpha}(0) = \alpha \mathbbm{1}$ did not achieve the specified convergence. $\boldsymbol{\beta}^{\star}$ denotes $\boldsymbol{\beta}_{\boldsymbol{w}}$, the ground truth vector with which the response data is generated.}\label{fig:l1error}
\end{figure}
...
In Figure \ref{fig:l1error}, we report the $\ell^1$ population error of the gradient descent solution. Here, \enquote{population error} refers to the distance between the gradient descent solution $\boldsymbol{\beta}_{\boldsymbol{w}(t_{\text{end}})} \in \mathbb{R}^n$ and the true vector $\boldsymbol{\beta}_{\boldsymbol{w}}$ with which the response data $y$ is generated. We plot the population error both with and without a logarithmic scale so that we can compare our results with those of Woodworth and colleagues. 
