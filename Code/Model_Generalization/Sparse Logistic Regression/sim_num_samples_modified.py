...
def sim_num_samples(threshold=0.6, d_list=[20, 40, 80, 160, 320, 640], num_trials=20, num_epochs=2e4, init_scale=0.1, num_units=10, num_test_samples=1e3, print_progress=True):
    ...
    if print_progress:
        f = open("progress_modified.txt", "w")
        f.write(f"sim_num_samples_modified.py,\td={d_list},\tthreshold={threshold}:\n")
        f.write("*----*----*----*----*----*----*----*----*----*----*----*----*----*\n")
...
if __name__ == "__main__":
    ...
    smallest_N = sim_num_samples(threshold=0.6, d_list=[20, 40, 80, 160, 320, 640], num_trials=10, num_epochs=1e4)
